...
/**
 * ThreeScene Component - Interactive 3D architectural visualization
 *
 * A comprehensive 3D facility viewer built with React Three Fiber and Three.js
 * that showcases the LawnTech Dynamics autonomous sports facility across 4 floors.
 *
 * Features:
 * - Full 3D models of all facility levels and sports courts
 * - Interactive floor selection and navigation
 * - Annotation modes: Clean view, Labels overlay, Measurement dimensions
 * - Smooth camera animations when switching floors
 * - Orbital controls for free 3D exploration
 * - Responsive lighting and shadows
 * - Interactive markers for key features
 * - Complex geometry with performance optimization (useMemo)
 *
 * The scene contains:
 * - Ground Floor: 24 tennis courts (Hard, Clay, Grass, Wood)
 * - Level 1: 16 Badminton + 4 Squash + 16 Table Tennis courts
 * - Level 2: 8 Pickleball courts + 1 Real Tennis court
 * - Level 3: 4 vertical farm sections with autonomous systems
 * - Outdoor plaza with landscaping
 * - Building shell with organic architecture and solar roof
 *
 * @component
 * @example
 * ```tsx
 ...
 * @param {Function} props.onFeatureSelect - Callback when user clicks a facility feature marker
 * @returns {React.ReactElement} Full-screen 3D scene viewer
 */

/**
 * Annotation display mode for the 3D scene
 * @typedef {('NONE' | 'LABELS' | 'MEASUREMENTS')} AnnotationMode
 */
type AnnotationMode = 'NONE' | 'LABELS' | 'MEASUREMENTS';

/**
 * Floor level selector
 * @typedef {('ALL' | 0 | 1 | 2 | 3)} FloorLevel
 * - 'ALL': Shows full facility overview
 * - 0: Ground floor (tennis courts)
 * - 1: Level 1 (racquet sports)
 * - 2: Level 2 (pickleball & heritage)
 * - 3: Level 3 (vertical farming)
 */
...
const FEATURES: FeatureData[] = [
  {
    ...
    description: '24 Courts: 6 Hard, 6 Clay, 6 Grass, 6 Wood.',
    icon: '🎾',
    position: [0, 5, 20],
  },
  {
  ...
  },
  {
    ...
    description: '4x 500sqm Autonomous Farming Sectors.',
    icon: '🌱',
    position: [0, 65, 0],
  },
  {
    id: 'outdoor_plaza',
    title: 'Outdoor Plaza',
    description: 'Public courts and relaxation zones.',
    icon: '🌳',
    position: [80, 0, 80],
  },
];
  ...
  return (
    <group position={position}>
      ...
      <Html distanceFactor={80} zIndexRange={[100, 0]} style={{ pointerEvents: 'none' }}>
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${isSelected ? 'bg-black animate-pulse' : 'bg-tennis-yellow'}`}
          />
          {title}
        </div>
      </Html>
...
const GroundFloor = ({
  ...
  for (let i = 0; i < 24; i++) {
    ...
    const col = i % 6;
    courts.push(
      <TennisCourt key={i} type={type} position={[-35 + col * 14, 0.1, -40 + row * 26]} />
    );
  }
      ...
      {Array.from({ length: 4 }).map((_, i) => (
        <group key={`s${i}`} position={[0, 2, -20 + i * 12]}>
          <mesh>
            ...
            <meshPhysicalMaterial transmission={0.6} roughness={0.1} thickness={0.5} color="#fff" />
          </mesh>
        </group>
      ))}
      {Array.from({ length: 16 }).map((_, i) => (
...
/**
 * Props for ThreeScene component
 * @interface ThreeSceneProps
 * @property {Function} onFeatureSelect - Callback triggered when user clicks on a feature marker
 */
interface ThreeSceneProps {
...
/**
 * Main ThreeScene component - renders the interactive 3D facility viewer
 * Manages scene state including floor level, annotation mode, and feature selection
 *
 * @param {ThreeSceneProps} props - Component props
 * @returns {React.ReactElement} Full-screen 3D canvas viewer
 */
const ThreeScene: React.FC<ThreeSceneProps> = ({ onFeatureSelect }) => {
  /** Currently selected feature marker ID */
  const [selectedId, setSelectedId] = useState<string | null>(null);

  /** Currently displayed floor level */
  const [activeFloor, setActiveFloor] = useState<FloorLevel>('ALL');

  /** Current annotation display mode */
  const [annotationMode, setAnnotationMode] = useState<AnnotationMode>('LABELS');

  /** Reference to OrbitControls for smooth camera animations */
  const controlsRef = useRef<any>(null);

  /** Flag to track if camera is currently animating between floors */
  const isAnimatingRef = useRef(false);

  /**
   * Handler for feature marker selection
   * Updates selected feature and automatically navigates to that floor level
   * @param {FeatureData} feature - The selected feature
   */
  const handleSelect = (feature: FeatureData) => {
    setSelectedId(feature.id);
    onFeatureSelect(feature);

    // Auto-navigate to the floor containing the selected feature
  ...
  /** Computed flag: show labels annotation when in LABELS mode */
  const showLabels = annotationMode === 'LABELS';

  /** Computed flag: show dimension measurements when in MEASUREMENTS mode */
