...
const ClayCourtEffect: React.FC<ClayCourtEffectProps> = ({
  ...
  const clayTexture = useMemo(() => {
    ...
    const baseColor = '#ea580c'; // Orange clay
  ...
  const normalMap = useMemo(() => {
    ...
    // Create subtle bumps for clay texture
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Position within court bounds
      positions[i3] = (Math.random() - 0.5) * width;
      positions[i3 + 1] = Math.random() * 0.5; // Low height
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      // Update lifetime for opacity fade
