...
export const LightingSystem: React.FC<{
  ...
  const handleConfigChange = (update: Partial<LightingConfig>) => {
    setConfig(prev => ({ ...prev, ...update }));
  };

  const floodlightPositions = useMemo(() => generateFloodlightPositions(), []);
  const courtLightPositions = useMemo(() => generateCourtLightPositions(), []);
  const ambientLightPositions = useMemo(() => generateAmbientLightPositions(), []);

  const floodlightsActive = config.floodlightsEnabled &&
    (config.mode === 'sports' || config.mode === 'event');
  const courtLightsActive = config.courtLightsEnabled &&
    (config.mode === 'sports' || config.mode === 'event');
  const ambientActive = config.mode !== 'natural';

  return (
    <>
      {isAssetEnabled('directional-light') && (
        <CelestialLight timeOfDay={config.timeOfDay} quality={config.quality} />
      )}

      <VolumetricFog timeOfDay={config.timeOfDay} density={config.fogDensity} />

      {isAssetEnabled('spot-lights') && floodlightPositions.map((fixture, idx) => (
        <FloodlightFixture
          key={`flood-${idx}`}
          position={fixture.position}
          ...
          color={fixture.color}
          distance={fixture.distance}
          angle={fixture.angle || Math.PI / 4}
          penumbra={fixture.penumbra || 0.3}
          enabled={floodlightsActive}
          quality={config.quality}
        />
      ))}

      {courtLightPositions.map((fixture, idx) => (
        <CourtSpotlight
          key={`court-${idx}`}
          position={fixture.position}
          intensity={fixture.intensity}
          ...
          distance={fixture.distance}
          angle={fixture.angle || Math.PI / 4}
          penumbra={fixture.penumbra || 0.2}
          enabled={courtLightsActive}
        />
      ))}

      <AmbientLightGrid
        positions={ambientLightPositions}
