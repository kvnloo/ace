...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, TrendingUp } from 'lucide-react';
...
const HeatMapOverlay: React.FC<HeatMapOverlayProps> = ({
  ...
  }, [courtId, dataType, timeRange, onPatternDetected]);
      ...
      for (let dy = -radius; dy <= radius; dy++) {
        for (let dx = -radius; dx <= radius; dx++) {
          ...
          if (x >= 0 && x < resolution && z >= 0 && z < resolution) {
            ...
            data[idx] = Math.min(255, data[idx] + contribution * 255);
          }
        }
      }
    });
  ...
  useFrame((_, delta) => {
    if (isPlaying && mode === 'historical') {
      const newTime = currentTime + delta * 1000 * playbackSpeed.current * 60;
